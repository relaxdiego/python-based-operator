...
clean:
	@helm uninstall --namespace=$(namespace) ${CHART_RELEASE_NAME} || echo "Prometheus Operator not installed. Ignoring"
	@# WARNING: DO NOT do this in a production environment as it will also delete the objects that are based on this CRD
	@helm uninstall prometheus-cluster-crd || echo 'prometheus-cluster-crd not installed. Ignoring.'; fi
...
debug:
	helm install --debug --dry-run ${CHART_RELEASE_NAME} charts/prometheus-operator/ 2>&1 | tee .last-helm-install
	helm install --debug --dry-run prometheus-cluster-crd charts/prometheus-cluster-crd/ 2>&1 | tee .last-helm-install
...
dev-operator: dependencies
	...
	helm install --atomic --set dev=true prometheus-cluster-crd charts/prometheus-cluster-crd/ 2>&1 | tee .last-helm-install
	helm install --atomic --set dev=true --namespace=$(namespace) --create-namespace ${CHART_RELEASE_NAME} charts/prometheus-operator/ 2>&1 | tee --append .last-helm-install
...
.last-helm-install: .last-docker-push
	...
	helm install --atomic --set dev=true prometheus-cluster-crd charts/prometheus-cluster-crd/ 2>&1 | tee .last-helm-install
	helm install --atomic --set dev=$(dev) --set image.repository=$(tag) --namespace=$(namespace) --create-namespace ${CHART_RELEASE_NAME} charts/prometheus-operator/ 2>&1 | tee .last-helm-install
